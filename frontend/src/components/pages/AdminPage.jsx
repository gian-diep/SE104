              ...
              <div key={listing.id} className="bg-white rounded-2xl border border-teal-100 hover:shadow-card shadow-soft overflow-hidden transition-all">
                <div className="grid grid-cols-12 gap-0">
                  ...
                  <div className="col-span-12 md:col-span-3 border-t md:border-t-0 md:border-l border-teal-100 flex flex-col">
                    ...
                    <button
                      onClick={() => setDeleteModal({ open: true, listingId: listing.id, listingName: listing.item_name })}
                      disabled={isBusy}
          ...
          <div className="relative w-full max-w-md overflow-hidden rounded-[2rem] border border-red-100 bg-white shadow-[0_20px_60px_rgba(0,0,0,0.18)] animate-in zoom-in-95 duration-200">
            ...
            <div className="relative px-7 pt-7 pb-5 border-b border-red-100 bg-gradient-to-b from-red-50/70 to-white">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="font-heading text-xl uppercase tracking-wide text-gray-900">Xóa bài đăng</h3>
                  <p className="mt-1 text-sm text-muted-foreground leading-relaxed">Bài đăng sẽ bị xóa vĩnh viễn và không thể khôi phục.</p>
                </div>
              </div>
            </div>
            <div className="px-7 py-6">
              <p className="text-sm text-gray-600">Bạn có chắc muốn xóa bài đăng này không?</p>
              <div className="mt-4 rounded-2xl border border-red-100 bg-red-50/50 p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="text-xs uppercase tracking-widest text-red-500 font-semibold">Bài đăng sẽ bị xóa</p>
                    <p className="truncate font-heading text-sm uppercase text-gray-900">{deleteModal.listingName}</p>
                  </div>
                </div>
              </div>
            </div>
            <div className="flex justify-end gap-3 border-t border-gray-100 bg-gray-50/60 px-7 py-5">
              ...
              <button
                ...
                {busy[deleteModal.listingId] ? 'Đang xóa...' : 'Xóa bài'}
              </button>
            </div>
          </div>
        </div>
...
function RatingStars({ value }) {
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      <span className="ml-1 text-xs font-mono text-amber-500 font-medium">{rating.toFixed(1)}</span>
    </div>
  )
}
...
function ReportCard({ report, onResolve, onPunish }) {
  ...
  const handlePunish = async () => {
    if (!punishAction) { alert('Vui lòng chọn hành động'); return }
    setPunishBusy(true)
    try {
      await punishUserFromReport(report.id, punishAction, punishNote)
      setPunishOpen(false)
      setPunishNote('')
      setPunishAction('')
      onPunish?.(report.id)
      ...
      <div className="p-5 pl-6">
        ...
        <div className="flex items-start justify-between gap-4 mb-4">
          <div className="flex items-start gap-3 min-w-0">
            <div className={`mt-0.5 w-8 h-8 flex-shrink-0 flex items-center justify-center rounded-xl ${isHighRisk && !resolved ? 'bg-red-100 text-red-500' : 'bg-teal-50 text-teal-500'}`}>
              <Flag className="w-4 h-4" />
            </div>
            <div className="min-w-0">
              <div className="mb-1">
                <span className="text-[10px] font-heading uppercase tracking-widest text-muted-foreground mr-2">Lý do:</span>
                <span className="font-heading text-sm uppercase tracking-wide text-foreground leading-snug">{report.reason}</span>
              </div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {banLabel && (
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-heading uppercase tracking-widest bg-red-500/10 text-red-600 border border-red-500/20">
                    <Ban className="w-3 h-3" />{banLabel}
                  </span>
                )}
              </div>
            </div>
        ...
        {report.detail && (
          <div className="mb-4">
            <p className="text-[11px] font-heading uppercase tracking-widest text-muted-foreground mb-2">Nội dung báo cáo</p>
            <div className="rounded-xl border border-teal-100 bg-teal-50/30 px-4 py-3">
              <p className="text-sm text-foreground whitespace-pre-wrap leading-relaxed">{report.detail}</p>
            </div>
          </div>
        )}
        ...
        {report.images && report.images.length > 0 && (
          <div className="mb-4">
            <p className="text-[11px] font-heading uppercase tracking-widest text-muted-foreground mb-2">Ảnh bằng chứng</p>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {report.listing_id && (
            <Link to={`/listings/${report.listing_id}`} target="_blank"
              ...
              <ExternalLink className="w-3 h-3" />Xem bài đăng
            </Link>
          )}
          <Link to={`/nguoi-dung/${report.reported_user_id}`} target="_blank"
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-teal-100 bg-white text-[11px] font-heading uppercase tracking-widest text-muted-foreground hover:text-primary hover:border-primary/30 shadow-soft transition-colors duration-150">
            <UserCircle className="w-3 h-3" />Xem tài khoản
          </Link>

          {!resolved && (
            <button onClick={() => setPunishOpen(true)}
              ...
              <AlertTriangle className="w-3 h-3" />Xử phạt
            </button>
          )}
          ...
          {!resolved && (
            <button onClick={() => onResolve(report.id)}
              ...
              <CheckCircle className="w-3 h-3" />Đánh dấu xử lý
            </button>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
          <div className="relative w-full max-w-md overflow-hidden rounded-2xl border border-orange-100 bg-white shadow-2xl">
            ...
            <div className="px-7 py-6 space-y-4">
              <div>
                ...
                <div className="space-y-2">
                  {[
                    { value: 'warn',          label: 'Gửi cảnh cáo',   desc: 'Không hạn chế tài khoản, chỉ ghi nhận vi phạm', icon: AlertTriangle, iconClass: 'text-yellow-500' },
                    { value: 'ban_7days',     label: 'Ban 7 ngày',     desc: 'Tài khoản bị hạn chế trong 7 ngày',             icon: ShieldAlert,   iconClass: 'text-orange-500' },
                    { value: 'ban_permanent', label: 'Ban vĩnh viễn',  desc: 'Tài khoản bị khoá không thời hạn',              icon: ShieldX,       iconClass: 'text-red-500'    },
                  ].map(opt => {
                    const Icon = opt.icon
                    return (
                      <label key={opt.value}
                        className={`flex items-start gap-3 p-3 border rounded-xl cursor-pointer transition-all ${
                          punishAction === opt.value ? 'border-orange-400 bg-orange-50' : 'border-gray-200 hover:border-orange-200 hover:bg-orange-50/50'
                        }`}>
                        <input type="radio" name="punishAction" value={opt.value}
              ...
              <div>
                ...
                <textarea value={punishNote} onChange={e => setPunishNote(e.target.value)}
                  placeholder="Lý do xử phạt, bằng chứng vi phạm..." rows={3}
            ...
            <div className="flex gap-3 border-t border-gray-100 bg-gray-50/60 px-7 py-5">
              <button onClick={() => { setPunishOpen(false); setPunishAction(''); setPunishNote('') }}
              ...
              <button onClick={handlePunish} disabled={!punishAction || punishBusy}
      ...
      {displayed.length === 0 ? (
        ...
        <div className="space-y-3">
          {displayed.map(report => (
            <ReportCard key={report.id} report={report} onResolve={updateStatus} onPunish={handlePunish} />
          ))}
        </div>
      )}
    </div>
...
function UsersTab() {
  ...
  return (
    <div>
          ...
          filtered.map((user, i) => (
            <div key={user.id}
              ...
              <div className="col-span-12 md:col-span-2 flex items-center gap-1 justify-end">
                <button onClick={() => setSelectedUser(user)} title="Xem chi tiết"
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-200">
          <div className="relative w-full max-w-md overflow-hidden rounded-[2rem] border border-red-100 bg-white shadow-[0_20px_60px_rgba(0,0,0,0.18)] animate-in zoom-in-95 duration-200">
            ...
            <div className="px-7 pt-8 pb-6 text-center">
              ...
              <h3 className="mt-5 font-heading text-2xl uppercase tracking-wide text-gray-900">Xóa người dùng</h3>
              <p className="mt-2 text-sm leading-relaxed text-gray-500">Bạn có chắc chắn muốn xóa tài khoản này không?</p>
              <div className="mt-5 rounded-[1.5rem] border border-red-100 bg-red-50/50 p-4 text-left">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-red-500">Người dùng sẽ bị xóa</p>
                    <p className="truncate font-heading text-sm uppercase text-gray-900">{deleteModal.userName}</p>
                  </div>
                </div>
              </div>
              <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-left">
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-xs leading-relaxed text-amber-800">Hành động này không thể hoàn tác và toàn bộ dữ liệu liên quan có thể bị mất.</p>
                </div>
              </div>
            </div>
            <div className="flex gap-3 border-t border-gray-100 bg-gray-50/60 px-7 py-5">
              <button onClick={() => setDeleteModal({ open: false, userId: null, userName: '' })}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-200">
          <div className="relative w-full max-w-md overflow-hidden rounded-[2rem] border border-orange-100 bg-white shadow-[0_20px_60px_rgba(0,0,0,0.18)] animate-in zoom-in-95 duration-200">
            ...
            <div className="px-7 pt-8 pb-6">
              ...
              <h3 className="mt-5 font-heading text-2xl uppercase tracking-wide text-gray-900 text-center">Cấm tài khoản</h3>
              ...
              <div className="mt-5 space-y-1.5">
                ...
                <textarea rows={3} value={banModal.reason}
            ...
            <div className="flex gap-3 border-t border-gray-100 bg-gray-50/60 px-7 py-5">
              <button onClick={() => setBanModal({ open: false, userId: null, userName: '', reason: '' })}
              ...
              <button onClick={confirmBan} disabled={!banModal.reason.trim() || busy[banModal.userId]}
      ...
      {selectedUser && (
        <>
          <div className="fixed inset-0 z-40 bg-black/50 backdrop-blur-sm" onClick={() => setSelectedUser(null)} />
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
            ...
            >
              ...
              <div className="px-6 pb-6 flex gap-3">
                ...
                <a href={`/nguoi-dung/${selectedUser.id}`} target="_blank" rel="noreferrer"
            ...
            <div key={appeal.id} className="bg-white border border-teal-100 rounded-2xl overflow-hidden shadow-soft hover:shadow-card transition-all">
              <div className="p-6 space-y-4">
                ...
                {isPending && (
                  <div>
                    {!isOpen ? (
                      <button onClick={() => setReviewOpen(r => ({ ...r, [appeal.id]: true }))}
                      ...
                      <div className="space-y-3 pt-2 border-t border-teal-100">
                        ...
                        <div className="flex items-center gap-3">
                          <button onClick={() => handleReview(appeal.id, 'approve')} disabled={busy[appeal.id]}
                          ...
                          <button onClick={() => handleReview(appeal.id, 'reject')} disabled={busy[appeal.id]}
                          ...
                          <button onClick={() => setReviewOpen(r => ({ ...r, [appeal.id]: false }))}
      ...
      <header className="border-b border-teal-100 bg-background sticky top-0 z-50 shadow-soft">
        <div className="max-w-[120rem] mx-auto px-6 md:px-12 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/"
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary/10 border border-primary/20 text-primary font-heading text-xs uppercase tracking-widest hover:bg-primary hover:text-primary-foreground hover:border-primary transition-all duration-200 shadow-soft hover:shadow-card">
              <ExternalLink className="h-4 w-4" />Về trang người dùng
            </Link>
